...
def _by_size(loc: Path, instances: list, config_dir: Path | None) -> list:
    """Instances smallest-first, not the alphabetical order the globs produce.

    Alphabetical front-loads the expensive half of a sweep -- custom_10_* sorts
    before custom_5_* -- so a run that is interrupted, or simply read partway
    through, has covered the large instances and none of the cheap ones.

    Size is number_of_trains from the config, else the scenario's own arrivals
    plus already-standing units (inStanding trains still have to be routed out,
    so they count). An instance neither answers for sorts last, not first: an
    unknown size is not evidence of a small one. Ties stay alphabetical.
    """
...
def main() -> None:
    ...
    # After the missing-scenario check, so that error still lists instances in
